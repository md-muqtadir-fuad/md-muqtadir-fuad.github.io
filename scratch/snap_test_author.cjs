...
async function main() {
  try {
    ...
    const fileUrl = 'file:///' + path.resolve('scratch/test_author_layout.html').replace(/\\/g, '/');
    await sendCommand('Page.navigate', { url: fileUrl });
    await new Promise(r => setTimeout(r, 500));

    const snap = await sendCommand('Page.captureScreenshot', { format: 'png' });
    fs.writeFileSync('scratch/test_author_layout_snap.png', Buffer.from(snap.data, 'base64'));
